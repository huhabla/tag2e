...
/**
 * \brief This abstract class is the base class for all numerical
 ...
 * the input data. This might be a linear regression model to
 * describe the connection between precipitation and temperature over space
 ...
 * 
 * Therefore the subclasses should implement general abstractions from statistical or numerical
 ...
 * "temp" is the name of the resulting data array in the vtkDataSet output.
 * 
 * vtkTAG2EAbstractModel expects a vtkDataSet as input and generates
 * a vtkDataSet as output. Two methods are provided to specify the model parameter:
 * <ul>
...
class vtkTAG2EAbstractModel : public vtkDataSetAlgorithm {
public:
    vtkTypeRevisionMacro(vtkTAG2EAbstractModel, vtkDataSetAlgorithm);
    static vtkTAG2EAbstractModel *New(); 

    //!\brief Set a single model parameter which
    //! should be used to process the input data
    ...
    //! the input data set on/off. Default is off.
